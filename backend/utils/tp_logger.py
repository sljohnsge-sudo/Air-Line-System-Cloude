...
Session cookies are redacted before being written to disk. Bearer tokens and
access_token response values are written in full, unredacted — per explicit
request, so Travelport's own team can inspect the real, original token value
directly in the saved logs (e.g. to confirm the same 24h-cached token was
reused across every call, per their own documented requirement to cache and
reuse rather than re-request per call).
"""
